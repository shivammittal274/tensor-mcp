 ...
 */
export async function extractPdfText(base64Data, filename) {
    try {
 ...
 */
export async function extractDocxText(base64Data, filename) {
    try {
        // Dynamically import to avoid loading cost unless needed
        const mammoth = await import('mammoth');
        const buffer = Buffer.from(base64Data, 'base64');
        const result = await mammoth.extractRawText({ buffer });
        const messages = (result.messages || []).map((m) => `- ${m.message || m.value || JSON.stringify(m)}`).join('\n');
 ...
 */
export async function extractXlsxText(base64Data, filename) {
    try {
