...
function base64UrlToBase64(input) {
    ...
    if (padLen === 2)
        ...
        output += '==='; // extremely rare, but safe guard
    return output;
}
 ...
 */
async function warmupContactSearch(peopleClient, contactType) {
    try {
...
function extractAccessToken(req) {
    ...
    if (!authData && req.headers['x-auth-data']) {
        try {
            authData = Buffer.from(req.headers['x-auth-data'], 'base64').toString('utf8');
        }
        catch (error) {
        ...
        try {
            switch (name) {
                ...
                case "gmail_read_email": {
                    ...
                    const emails = threadMessages.map((msg) => {
                ...
                case "gmail_get_email_attachments": {
                    ...
                    collectAttachments(messageResponse.data.payload);
                    ...
                    const attachmentContents = await Promise.all(attachmentsMeta.map(async (meta) => {
                ...
                case "gmail_search_contacts": {
                    ...
                    try {
                        ...
                        if (contactType === 'all') {
                            ...
                            const personalResults = (personalRes.data.results || []).map((result) => {
                            ...
                            const otherResults = (otherRes.data.results || []).map((result) => {
                                const person = result.person || {};
                                const names = person.names || [];
                                const emails = person.emailAddresses || [];
                                const phones = person.phoneNumbers || [];
                            ...
                            const directoryResults = (directoryRes.data.people || []).map((person) => {
                                const names = person.names || [];
                                const emails = person.emailAddresses || [];
                                const phones = person.phoneNumbers || [];
                                const orgs = person.organizations || [];
                            ...
                            const resultPayload = {
                                ...
                                personal: {
                                    ...
                                    nextPageToken: personalRes.data.nextPageToken || undefined,
                                    contacts: personalResults,
                                },
                                other: {
                                    resultCount: otherResults.length,
                                    nextPageToken: otherRes.data.nextPageToken || undefined,
                                    contacts: otherResults,
                                },
                                directory: {
                                    resultCount: directoryResults.length,
                                    nextPageToken: directoryRes.data.nextPageToken || undefined,
                            ...
                            results = (response.data.people || []).map((person) => {
                                const names = person.names || [];
                                const emails = person.emailAddresses || [];
                                const phones = person.phoneNumbers || [];
                                const orgs = person.organizations || [];
